...
class Employee(object):
    ...
    def __init__(self, excel_filename="shift.xlsm"):
    ...
    def read_excel(self):
        ...
        self.name = df.iloc[4:16,2:3]
        self.no = df.iloc[4:16,0:1]
        self.manager = df.iloc[4:16,1:2].fillna(0).replace("◎",1)

        df_read_excel = df.iloc[4:16,3:35]
        df_read_excel = df_read_excel.fillna(0)
        df_read_excel = df_read_excel.replace("○",1)

        #希望出勤数
        work_day = df_read_excel.iloc[:,33:34].reset_index(drop=True)
        work_day.colums = ["出勤希望"]

        #出勤可能な日
        wills = df_read_excel.iloc[:,0:31].reset_index(drop=True)
        wills.columns = [i+1 for i in range(len(kiso.columns))]
        self.wills = wills
...
#内部的には２（朝昼）* 人　＊10人　＝３００
    def make_sample(self):
        sample_list = [][]
        num_employee = 2 * self.wills.columns * len(self.wills)
    ...
    def slice(self):
        ...
        start = 0
        for num in range(10):
          sliced.append(self.list[start:(start + len(self.wills))])
          start = start + len(self.wills)
        return tuple(sliced)
